...
export function LeadsAreaChart() {
    ...
    const fetchLeadsData = async () => {
        try {
            ...
            const leadsByDate = new Map<string, { renataOriginal: number, renataCardoso: number }>()

            data?.forEach(lead => {
                const dateStr = parseDate(lead.created_at);
                const currentCounts = leadsByDate.get(dateStr) || { renataOriginal: 0, renataCardoso: 0 };
